...
(
	...
	token_expired_date datetime null,									-- 토큰 만료일
...
-- History 관리는 LogDB에서 해당 테이블은 유저가 로그인시 체크용
...
(
    ...
    user_idx 		bigint                  not null,   				-- 신고한 유저
    target_idx      bigint                  not null,   				-- 신고당한 유저
    report_reason   tinyint                 not null,   				-- 0-욕설, 1-핵, 2-비매너 ...
    report_detail   nvarchar(500)           null,						-- 신고 상세 사유 
    [status]        tinyint                 not null DEFAULT 0, 		-- 0-접수, 1-처리완료, 2-반려 ...
    created_date    datetime                not null DEFAULT getdate()	-- 신고일
)
...
(
	...
	[status] tinyint not null,											-- 결제 상태 (0 - 진행중, 1 - 완료, 2 - 실패 ...)
	product_id int not null,											-- 상품ID
	country_code char(3) NOT NULL DEFAULT 'zz',							-- 국가코드 (default zz)
	amount decimal(10,2) not null,										-- 금액
	currency nvarchar(5) not null default 'zz',							-- 화폐 코드
	receipt nvarchar(300),												-- 영수증 // 길이 더 늘려야 할수도
	bill_tx_id nvarchar(300),											-- 스토어 결제 고유id
...
CREATE INDEX ix_billing_user_idx ON billing(user_idx)          			-- 유저 결제 이력/상태 조회
...
(
	...
	is_active tinyint not null default 0,								-- 공지 활성화 여부( 0 - 비활성화, 1- 활성화)
	notice_start_date datetime not null default '2000-01-01',			-- 공지 시작일 
	notice_end_date datetime not null default '2000-01-01',				-- 공지 종료일
	created_date datetime not null default getdate()					-- 공지 작성일
)
-- Row가 많지 않을것이고 값이 추가되거나 할일도 별로 없을것으로 예상
-- 풀스캔해도 문제 없을것이고 인덱스 없이 필터링해도 문제 없을듯.
-- 로우가 좀 많아져도 그때 인덱스추가해도 될듯하다.